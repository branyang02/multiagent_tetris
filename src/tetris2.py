...
class Tetris2:
    ...
    def garbage(self, cleared_lines, b2b):
        # self.board = [[0] * self.width for _ in range(self.height)]
        
        # for i in range(len(self.board[0])):
        #     self.board[0][i] = 0
        num_garbage = cleared_lines - 1
        if cleared_lines == 4:
            num_garbage = 4
        if cleared_lines == 4 and b2b != 0:
            num_garbage += b2b
            print("b2b %d player 2" % b2b)
        ...
        well = random.randint(0, self.width - 1)
        for i in range(1, num_garbage + 1):
            garbage = self.width * [8]
            garbage[well] = 0
            temp_board[-i] = garbage
        # temp_board[-1] = [5,5,5,5,0,5,5,5,5,5]
        # temp_board[-2] = [5,5,5,5,0,0,5,5,5,5]

        # print(temp_board[-1])
        # print(temp_board[-2])
        self.board = list(temp_board)
